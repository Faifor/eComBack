...
Swagger (`/docs`) intentionally shows **canonical frontend contracts only**:
- admin UI: `/admin/*`
- user/storefront: `/auth/*`, `GET /catalog/*`, reviews, `/cart/*`, `/orders/*`

Overlapping low-level admin-capable endpoints in `/catalog/*` and `/pricing/*` are kept for compatibility but hidden from schema to avoid duplication confusion.
